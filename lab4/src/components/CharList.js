...
const CharList = (props) => {
		...
		() => {
			...
			async function fetchData() {
				try {
                    ...
                    const hash = md5(stringToHash);
                    const baseUrl = 'https://gateway.marvel.com:443/v1/public/characters';
                    const url = baseUrl + '?nameStartsWith=' + searchTerm + '&ts=' + ts + '&apikey=' + publickey + '&hash=' + hash + '&limit=' + cardsPerPage;
                    const data = await axios.get(url);
					...
					<CardActionArea>
						<Link to={`/characters/${char.id}`}>
							<CardMedia
								...
								component='img'
								image={char.thumbnail.path ? char.thumbnail.path + "/portrait_xlarge." + char.thumbnail.extension : noImage}
								title='char image'
							/>
    ...
    if (error404){
        return (
			<div>
				<h2>Couldn't find any characters on this page, maybe try another</h2>
			</div>
		);
    }
